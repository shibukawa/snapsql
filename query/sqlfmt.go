...
func ensureSpaceAfterPlaceholders(sql string) string {
    ...
    for i := 0; i < len(sql); i++ {
        ch := sql[i]
        if ch == '\'' && !inDouble { inSingle = !inSingle; b.WriteByte(ch); continue }
        if ch == '"' && !inSingle { inDouble = !inDouble; b.WriteByte(ch); continue }
        ...
        if !inSingle && !inDouble {
            ...
            if ch == '$' {
                ...
                if j > i+1 {
                    ...
                    if j < len(sql) && isIdent(sql[j]) { b.WriteByte(' ') }
                    i = j - 1
                    continue
                }
            }
