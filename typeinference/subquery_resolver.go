...
func (r *SubqueryTypeResolver) createSubEngine(node *parser.SQDependencyNode) *TypeInferenceEngine2 {
...
func (r *SubqueryTypeResolver) extractAvailableTablesFromScope(node *parser.SQDependencyNode) []string {
	var tables []string
...
func (r *SubqueryTypeResolver) calculateSubqueryDepth(node *parser.SQDependencyNode) int {
...
func (r *SubqueryTypeResolver) GetAvailableSubqueryTables() []string {
	...
	for nodeID := range r.typeCache {
		if node := dependencyGraph.GetNode(nodeID); node != nil {
			if node.NodeType == parser.DependencyCTE {
