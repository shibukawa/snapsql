...
func TestGenerateDummyDataFromSchemaWithNullable(t *testing.T) {
	schema := &FunctionDefinition{
		Parameters: map[string]any{
			"name":      "string",
			"age":       "int",
	...
	dummy := generateDummyDataFromSchema(schema)

	// Verify non-nullable parameters are included
	assert.True(t, dummy["name"] != nil)
	assert.True(t, dummy["age"] != nil)
	assert.True(t, dummy["data"] != nil)

	// Verify nullable parameters are excluded
	_, hasOptional := dummy["optional"]
	assert.False(t, hasOptional)
	_, hasScore := dummy["score"]
	assert.False(t, hasScore)

	// Check nested data (should include non-nullable nested fields)
	dataMap, ok := dummy["data"].(map[string]any)
	assert.True(t, ok)
	assert.True(t, dataMap["nested"] != nil)

	// Nested nullable should also be excluded
	_, hasNestedNullable := dataMap["nullable"]
	assert.False(t, hasNestedNullable)
}
