...
func (q *QueryCmd) generateSQLFromTemplate(templateContent string, params map[string]any) (string, []any, error) {
	...
	sql, sqlParams, err := q.executeInstructions(format.Instructions, format.CELExpressions, params)
	if err != nil {
...
func (q *QueryCmd) executeInstructions(instructions []intermediate.Instruction, expressions []intermediate.CELExpression, params map[string]any) (string, []any, error) {
	...
	for _, instr := range instructions {
		...
		case intermediate.OpEmitEval:
			if instr.ExprIndex != nil && *instr.ExprIndex >= 0 && *instr.ExprIndex < len(expressions) {
				expr := expressions[*instr.ExprIndex].Expression
