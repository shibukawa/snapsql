...
func UnreadNotificationForUsers(ctx context.Context, executor snapsqlgo.DBExecutor, notificationID int, opts ...snapsqlgo.FuncOpt) (any, error) {
	...
	funcConfig := snapsqlgo.GetFunctionConfig(ctx, "unreadNotificationForUsers", "any")
	// Check for mock mode
	if funcConfig != nil && len(funcConfig.MockDataNames) > 0 {
		mockData, err := snapsqlgo.GetMockDataFromFiles(unreadNotificationForUsersMockPath, funcConfig.MockDataNames)
		if err != nil {
			return nil, fmt.Errorf("UnreadNotificationForUsers: failed to get mock data: %w", err)
		}

		result, err = snapsqlgo.MapMockDataToStruct[any](mockData)
		if err != nil {
			return nil, fmt.Errorf("UnreadNotificationForUsers: failed to map mock data to any struct: %w", err)
		}
	...
	buildQueryAndArgs := func() (string, []any, error) {
		query := "UPDATE inbox SET read_at = NULL, updated_at =$1  WHERE notification_id =$2"
		args := make([]any, 0)
		...
		evalRes1, _, err := unreadNotificationForUsersPrograms[0].Eval(paramMap)
		if err != nil {
	...
	if err != nil {
		return nil, fmt.Errorf("UnreadNotificationForUsers: failed to prepare statement: %w (query: %s)", err, query)
	}
	defer stmt.Close()
