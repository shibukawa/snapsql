...
func ListReorder(ctx context.Context, executor snapsqlgo.DBExecutor, listID int, position float64, opts ...snapsqlgo.FuncOpt) iter.Seq2[*ListReorderResult, error] {
	...
	buildQueryAndArgs := func() (string, []any, error) {
		query := "UPDATE lists SET position = $1, updated_at = NOW() , created_at = $2, created_by = $3, updated_by = $4  WHERE id = $5   RETURNING id, board_id, name, stage_order, position, is_archived, created_at, updated_at"
		args := make([]any, 0)
		...
		evalRes4, _, err := listReorderPrograms[1].Eval(paramMap)
		if err != nil {
			return "", nil, fmt.Errorf("ListReorder: failed to evaluate expression: %w", err)
		}
