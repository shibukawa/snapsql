...
func CardMove(ctx context.Context, executor snapsqlgo.DBExecutor, cardID int, targetListID int, targetPosition float64, opts ...snapsqlgo.FuncOpt) iter.Seq2[*CardMoveResult, error] {
	...
	buildQueryAndArgs := func() (string, []any, error) {
		query := "UPDATE cards SET list_id = $1, position = $2, updated_at = NOW() , created_at = $3, created_by = $4, updated_by = $5  WHERE id = $6   RETURNING id, list_id, title, description, position, created_at, updated_at"
		args := make([]any, 0)
		...
		evalRes5, _, err := cardMovePrograms[2].Eval(paramMap)
		if err != nil {
			return "", nil, fmt.Errorf("CardMove: failed to evaluate expression: %w", err)
		}
