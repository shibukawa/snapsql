...
func CardUpdate(ctx context.Context, executor snapsqlgo.DBExecutor, cardID int, title string, description string, opts ...snapsqlgo.FuncOpt) iter.Seq2[*CardUpdateResult, error] {
	...
	buildQueryAndArgs := func() (string, []any, error) {
		query := "UPDATE cards SET title = $1, description = $2, updated_at = NOW() , created_at = $3, created_by = $4, updated_by = $5  WHERE id = $6   RETURNING id, list_id, title, description, position, created_at, updated_at"
		args := make([]any, 0)
		...
		evalRes5, _, err := cardUpdatePrograms[2].Eval(paramMap)
		if err != nil {
			return "", nil, fmt.Errorf("CardUpdate: failed to evaluate expression: %w", err)
		}
