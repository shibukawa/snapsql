...
func CardReorder(ctx context.Context, executor snapsqlgo.DBExecutor, cardID int, position float64, opts ...snapsqlgo.FuncOpt) iter.Seq2[*CardReorderResult, error] {
	...
	buildQueryAndArgs := func() (string, []any, error) {
		query := "UPDATE cards SET position = $1, updated_at = NOW() , created_at = $2, created_by = $3, updated_by = $4  WHERE id = $5   RETURNING id, list_id, title, description, position, created_at, updated_at"
		args := make([]any, 0)
		...
		evalRes4, _, err := cardReorderPrograms[1].Eval(paramMap)
		if err != nil {
			return "", nil, fmt.Errorf("CardReorder: failed to evaluate expression: %w", err)
		}
	...
	return func(yield func(*CardReorderResult, error) bool) {
		...
		if err != nil {
			err = fmt.Errorf("CardReorder: failed to prepare statement: %w (query: %s)", err, query)
			logger.SetErr(err)
			_ = yield(nil, err)
			return
		}
		...
		if err != nil {
			err = fmt.Errorf("CardReorder: failed to execute query: %w", err)
			logger.SetErr(err)
			_ = yield(nil, err)
			return
		}
		...
		for rows.Next() {
			...
			if err := rows.Scan(
				...
				err = fmt.Errorf("CardReorder: failed to scan row: %w", err)
				logger.SetErr(err)
				_ = yield(nil, err)
				return
			}
		...
		if err := rows.Err(); err != nil {
			err = fmt.Errorf("CardReorder: error iterating rows: %w", err)
			logger.SetErr(err)
			_ = yield(nil, err)
			return
		}
