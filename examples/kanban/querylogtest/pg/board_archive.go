...
func BoardArchive(ctx context.Context, executor snapsqlgo.DBExecutor, opts ...snapsqlgo.FuncOpt) iter.Seq2[*BoardArchiveResult, error] {
	...
	buildQueryAndArgs := func() (string, []any, error) {
		query := "UPDATE boards SET status = 'archived', archived_at = NOW(), updated_at = NOW() , created_at = $1, created_by = $2, updated_by = $3  WHERE status = 'active'  RETURNING id, name, status, archived_at, created_at, updated_at"
		args := make([]any, 0)
