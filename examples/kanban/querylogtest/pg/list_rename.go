...
func ListRename(ctx context.Context, executor snapsqlgo.DBExecutor, listID int, name string, opts ...snapsqlgo.FuncOpt) iter.Seq2[*ListRenameResult, error] {
	...
	buildQueryAndArgs := func() (string, []any, error) {
		query := "UPDATE lists SET name = $1, updated_at = NOW() , created_at = $2, created_by = $3, updated_by = $4  WHERE id = $5   RETURNING id, board_id, name, stage_order, position, is_archived, created_at, updated_at"
		args := make([]any, 0)
		...
		evalRes4, _, err := listRenamePrograms[1].Eval(paramMap)
		if err != nil {
			return "", nil, fmt.Errorf("ListRename: failed to evaluate expression: %w", err)
		}
