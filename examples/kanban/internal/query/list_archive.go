...
func init() {
	...
	// Environment 0: Base environment
	{
		...
		opts := []cel.EnvOption{
			...
			cel.Variable("is_archived", cel.BoolType),
		}
...
func ListArchive(ctx context.Context, executor snapsqlgo.DBExecutor, listID int, isArchived bool, opts ...snapsqlgo.FuncOpt) iter.Seq2[*ListArchiveResult, error] {
	...
	buildQueryAndArgs := func() (string, []any, error) {
		query := "UPDATE lists SET is_archived =$1, updated_at =$2  WHERE id =$3  RETURNING id, board_id, name, stage_order, position, is_archived, created_at, updated_at"
		args := make([]any, 0)
	...
	return func(yield func(*ListArchiveResult, error) bool) {
		...
		if err != nil {
			_ = yield(nil, fmt.Errorf("ListArchive: failed to prepare statement: %w", err))
			return
		}
		defer stmt.Close()
