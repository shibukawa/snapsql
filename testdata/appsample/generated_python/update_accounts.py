...
_EXPLANG_EXPRESSIONS = [
    [
        ("identifier", "status", "", 0, False),
    ],
    [
        ("identifier", "include_filter", "", 0, False),
    ],
    [
...
async def update_account_status_conditional(
    ...
    args.append(_eval_explang_expression(0, param_map))
    sql_parts.append(" WHERE ")
    cond_value = _eval_explang_expression(1, param_map)
    if _truthy(cond_value):
        sql_parts.append(" id = ?")
        args.append(_eval_explang_expression(2, param_map))
        sql_parts.append(" ")
    sql_parts.append(" ")

    # Join SQL parts
    sql = ''.join(sql_parts)
    # WHERE clause safety check for MutationUpdate
    ctx = get_snapsql_context()
    where_meta = {
        'status': "conditional",
        'removal_combos': [
            [{'expr_index': 1, 'when': False}],
        ],
        'expression_refs': [1],
        'dynamic_conditions': [
            {'expr_index': 1, 'negated_when_empty': True, 'has_else': False, 'description': "include_filter"},
        ],
        'raw_text': "WHERE",
    }
    enforce_non_empty_where_clause(ctx, "UpdateAccountStatusConditional", "update", where_meta, sql)
