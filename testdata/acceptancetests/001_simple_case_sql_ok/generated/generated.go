...
// FindUser - sql.Result Affinity
func FindUser(ctx context.Context, executor snapsqlgo.DBExecutor, userID int, opts ...snapsqlgo.FuncOpt) (sql.Result, error) {
	var result sql.Result

	// Hierarchical metas (for nested aggregation code generation - placeholder)
	// Count: 0

	funcConfig := snapsqlgo.GetFunctionConfig(ctx, "finduser", "sql.result")
	// Check for mock mode
	if funcConfig != nil && len(funcConfig.MockDataNames) > 0 {
		mockData, err := snapsqlgo.GetMockDataFromFiles(finduserMockPath, funcConfig.MockDataNames)
		if err != nil {
			return nil, fmt.Errorf("FindUser: failed to get mock data: %w", err)
		}

		result, err = snapsqlgo.MapMockDataToStruct[sql.Result](mockData)
		if err != nil {
			return nil, fmt.Errorf("FindUser: failed to map mock data to sql.Result struct: %w", err)
		}
	...
	buildQueryAndArgs := func() (string, []any, error) {
		...
		evalRes0, _, err := finduserPrograms[0].Eval(paramMap)
		if err != nil {
			return "", nil, fmt.Errorf("FindUser: failed to evaluate expression: %w", err)
		}
	...
	if err != nil {
		return nil, fmt.Errorf("FindUser: failed to prepare statement: %w", err)
	}
	defer stmt.Close()
	// Execute query and scan multiple rows (many affinity)
	rows, err := stmt.QueryContext(ctx, args...)
	if err != nil {
		return nil, fmt.Errorf("FindUser: failed to execute query: %w", err)
	}
	defer rows.Close()
