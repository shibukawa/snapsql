...
func init() {
	...
	// Environment 0 (container: root)
	{
		...
		opts = append(opts, cel.Variable("values", cel.ListType(cel.IntType)))
		opts = append(opts,
	...
	// expr_001: "values" using environment 0
	{
		...
		if issues != nil && issues.Err() != nil {
			panic(fmt.Sprintf("failed to compile CEL expression %q: %v", "values", issues.Err()))
		}
		program, err := celEnvironments[0].Program(ast)
		if err != nil {
			panic(fmt.Sprintf("failed to create CEL program for %q: %v", "values", err))
		}
