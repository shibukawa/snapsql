...
// InsertUserWithReturningMysql - sql.Result Affinity
func InsertUserWithReturningMysql(ctx context.Context, executor snapsqlgo.DBExecutor, userName string, userEmail string, opts ...snapsqlgo.FuncOpt) (sql.Result, error) {
	...
	if funcConfig != nil && len(funcConfig.MockDataNames) > 0 {
		...
		if err != nil {
			return nil, fmt.Errorf("InsertUserWithReturningMysql: failed to map mock data to sql.Result struct: %w", err)
		}
	...
	if err != nil {
		return nil, fmt.Errorf("InsertUserWithReturningMysql: failed to prepare statement: %w", err)
	}
	defer stmt.Close()
	// Execute statement (no response struct available)
	execResult, err := stmt.ExecContext(ctx, args...)
	if err != nil {
		return nil, fmt.Errorf("InsertUserWithReturningMysql: failed to execute statement: %w", err)
	}
	result = execResult
