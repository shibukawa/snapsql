...
func GetComprehensiveDialectTest(ctx context.Context, executor snapsqlgo.DBExecutor, userID int, opts ...snapsqlgo.FuncOpt) (sql.Result, error) {
	...
	// Build SQL
	query := "SELECT id, name, (age::INTEGER) as age_cast_standard, price::DECIMAL(10,2) as price_cast_postgresql, (salary + bonus)::NUMERIC(12,2) as total_cast_complex, first_name || ' ' || last_name as full_name_mysql, first_name || ' ' || last_name as full_name_postgresql, CURRENT_TIMESTAMP as time_mysql, CURRENT_TIMESTAMP as time_standard, TRUE as bool_true, FALSE as bool_false, RANDOM() as random_mysql, RANDOM() as random_postgresql, (NOW()::TEXT) as nested_cast_time, 'ID: ' || CAST(id AS TEXT) as nested_concat_cast FROM users WHERE id =$1AND active = TRUE AND created_at > CURRENT_TIMESTAMP"
	args := []any{
		userID,
	}
