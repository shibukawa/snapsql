...
// GetUser - sql.Result Affinity
func GetUser(ctx context.Context, executor snapsqlgo.DBExecutor, userID int, user User, opts ...snapsqlgo.FuncOpt) (sql.Result, error) {
	var result sql.Result

	// Hierarchical metas (for nested aggregation code generation - placeholder)
	// Count: 0

	funcConfig := snapsqlgo.GetFunctionConfig(ctx, "getuser", "sql.result")
	// Check for mock mode
	if funcConfig != nil && len(funcConfig.MockDataNames) > 0 {
		mockData, err := snapsqlgo.GetMockDataFromFiles(getuserMockPath, funcConfig.MockDataNames)
		if err != nil {
			return nil, fmt.Errorf("GetUser: failed to get mock data: %w", err)
		}

		result, err = snapsqlgo.MapMockDataToStruct[sql.Result](mockData)
		if err != nil {
			return nil, fmt.Errorf("GetUser: failed to map mock data to sql.Result struct: %w", err)
		}
	...
	buildQueryAndArgs := func() (string, []any, error) {
		query := "SELECT u.id, u.name, u.email FROM users u  WHERE u.id =$1"
		args := make([]any, 0)
		...
		evalRes0, _, err := getuserPrograms[0].Eval(paramMap)
		if err != nil {
			return "", nil, fmt.Errorf("GetUser: failed to evaluate expression: %w", err)
		}
	...
	if err != nil {
		return nil, fmt.Errorf("GetUser: failed to prepare statement: %w", err)
	}
	defer stmt.Close()
	// Execute query and scan multiple rows (many affinity)
	rows, err := stmt.QueryContext(ctx, args...)
	if err != nil {
		return nil, fmt.Errorf("GetUser: failed to execute query: %w", err)
	}
	defer rows.Close()
