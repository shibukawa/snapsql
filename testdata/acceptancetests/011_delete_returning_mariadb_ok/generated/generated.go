...
func init() {
	...
	// Environment 0 (container: root)
	{
		...
		if err != nil {
			panic(fmt.Sprintf("failed to create DeleteUserWithReturningMariadb CEL environment 0: %v", err))
		}
...
// DeleteUserWithReturningMariadb - sql.Result Affinity
func DeleteUserWithReturningMariadb(ctx context.Context, executor snapsqlgo.DBExecutor, userID int, opts ...snapsqlgo.FuncOpt) (sql.Result, error) {
	...
	if funcConfig != nil && len(funcConfig.MockDataNames) > 0 {
		...
		if err != nil {
			return nil, fmt.Errorf("DeleteUserWithReturningMariadb: failed to map mock data to sql.Result struct: %w", err)
		}
	...
	if err != nil {
		return nil, fmt.Errorf("DeleteUserWithReturningMariadb: failed to prepare statement: %w", err)
	}
	defer stmt.Close()
	// Execute query and scan multiple rows (many affinity)
	rows, err := stmt.QueryContext(ctx, args...)
	if err != nil {
		return nil, fmt.Errorf("DeleteUserWithReturningMariadb: failed to execute query: %w", err)
	}
	defer rows.Close()
