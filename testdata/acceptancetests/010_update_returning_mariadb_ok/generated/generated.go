...
func UpdateUserWithReturningMariadb(ctx context.Context, executor snapsqlgo.DBExecutor, userID int, newName string, opts ...snapsqlgo.FuncOpt) (sql.Result, error) {
	...
	if funcConfig != nil && len(funcConfig.MockDataNames) > 0 {
		...
		if err != nil {
			return nil, fmt.Errorf("UpdateUserWithReturningMariadb: failed to map mock data to sql.Result struct: %w", err)
		}
	...
	if err != nil {
		return nil, fmt.Errorf("UpdateUserWithReturningMariadb: failed to prepare statement: %w", err)
	}
	defer stmt.Close()
	// Execute query (no result expected)
	execResult, err := stmt.ExecContext(ctx, args...)
	if err != nil {
		return nil, fmt.Errorf("UpdateUserWithReturningMariadb: failed to execute statement: %w", err)
	}
	result = execResult
