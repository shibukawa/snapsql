...
func init() {
	...
	// Environment 0 (container: root)
	{
		...
		opts = append(opts, cel.Variable("user_name", cel.StringType))
		opts = append(opts, cel.Variable("user_email", cel.StringType))
		opts = append(opts,
...
func InsertUserWithReturningSqlite(ctx context.Context, executor snapsqlgo.DBExecutor, userName string, userEmail string, opts ...snapsqlgo.FuncOpt) (sql.Result, error) {
	...
	if funcConfig != nil && len(funcConfig.MockDataNames) > 0 {
		...
		if err != nil {
			return nil, fmt.Errorf("InsertUserWithReturningSqlite: failed to get mock data: %w", err)
		}

		result, err = snapsqlgo.MapMockDataToStruct[sql.Result](mockData)
		if err != nil {
			return nil, fmt.Errorf("InsertUserWithReturningSqlite: failed to map mock data to sql.Result struct: %w", err)
		}
	...
	buildQueryAndArgs := func() (string, []any, error) {
		...
		evalRes0, _, err := insertUserWithReturningSqlitePrograms[0].Eval(paramMap)
		if err != nil {
			return "", nil, fmt.Errorf("InsertUserWithReturningSqlite: failed to evaluate expression: %w", err)
		}
		args = append(args, evalRes0.Value())

		evalRes1, _, err := insertUserWithReturningSqlitePrograms[1].Eval(paramMap)
		if err != nil {
			return "", nil, fmt.Errorf("InsertUserWithReturningSqlite: failed to evaluate expression: %w", err)
		}
	...
	if err != nil {
		return nil, fmt.Errorf("InsertUserWithReturningSqlite: failed to prepare statement: %w", err)
	}
	defer stmt.Close()
	// Execute statement (no response struct available)
	execResult, err := stmt.ExecContext(ctx, args...)
	if err != nil {
		return nil, fmt.Errorf("InsertUserWithReturningSqlite: failed to execute statement: %w", err)
	}
	result = execResult
