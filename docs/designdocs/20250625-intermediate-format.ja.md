...
##### 9.1 HierarchyKeyLevel フィールド

多段階層のレスポンス構造（`a__b__c__id` 形式など）を木構造へ正しくマッピングし、重複行の集約（親子 one-to-many 展開）を行うために `hierarchy_key_level` を各レスポンス列へ付与します。

| 値 | 意味 | 例（列名） |
|----|------|------------|
| 0 | 非キー列 | `a__b__value` など PK ではない列 |
| 1 | ルート(最上位)主キー列 | `id` / `user_id` |
| 2 | 第一階層子ノード主キー列 | `a__id` |
| 3 | 第二階層子ノード主キー列 | `a__b__id` |
| n | n-1 階層の子ノード主キー列 | `a__b__c__...__id` |

特性:
1. `hierarchy_key_level > 0` が「この列はその階層ノードを一意に識別するキー列」を意味します。
2. 旧フィールド `is_primary_key` は冗長化のため削除（2025-09）。今後は `hierarchy_key_level` のみを参照します。
3. 同一パス階層で複合主キーを構成する複数列は同じレベル値を持ちます（例: `id` と `tenant_id` がルート複合PKなら両方 1）。
4. ネストレベルはプレフィックス分割数に 1 を加えた深さと一致 (例: `a__b__id` -> プレフィックス `a`,`b` -> 深さ3 -> level=3)。
5. 解析段階で一つも level=1 が検出されない場合、安全策として最初の列を level=1 にフォールバックし木構築の破綻を防ぎます。

コード生成での利用:
1. ルートPK(level=1) の連結値で `_parentMap` （ルート行Map）をインデックス化
2. 子ノード毎に `path`（`a`, `a__b` など）単位の Map (`_nodeMap<StructName>`) を持ち、`chainKey|path:localKey` で重複判定
3. `len(node.Path)+1 == column.HierarchyKeyLevel` でそのノードに属するPK列か判定（`node.Path` はプレフィックス配列）
4. PK列集合が同じ行は append せず再利用、非キー列のみ値を上書き/設定

制約と注意:
- `hierarchy_key_level` は spec 上オプショナル(JSON出力は `omitempty`) だが、レベル>0 の列は生成結果で常に出力される想定。
- 将来的に階層抑止オプションを導入する場合もレベル付与自体は保持し、生成フェーズで無視する設計を推奨。
