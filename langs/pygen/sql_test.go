...
func TestGenerateStaticSQL(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			wantArgs:     []string{"username", "updated_by", "user_id"},
			wantIsStatic: true,
		},
	}
...
func TestGenerateDynamicSQL(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			wantIsStatic: false,
			wantCodeContains: []string{
				"id_collection = user_ids",
				"if id_collection is None:",
				"for id in id_iterable:",
				"args.append(id)",
			},
		},
		{
